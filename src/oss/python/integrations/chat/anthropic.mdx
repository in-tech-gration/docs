...
<Info>
    **AWS Bedrock and Google VertexAI**
    ...
    Note that certain Anthropic models can also be accessed via AWS Bedrock and Google VertexAI. See the [`ChatBedrock`](/oss/integrations/chat/bedrock/) and [`ChatVertexAI`](/oss/integrations/chat/google_vertex_ai/) integrations to use Anthropic models via these services.
</Info>
...
Anthropic supports opt-in [strict schema adherence to tool calls](https://docs.claude.com/en/docs/build-with-claude/structured-outputs). This guarantees that tool names and arguments are validated and correctly typed.

To enable strict tool use:
1. Specify the `structured-outputs-2025-11-13` beta header
2. Specify `strict=True` when calling [bind_tools](/oss/langchain/models#tool-calling).
...
Anthropic supports a [token-efficient tool use](https://docs.claude.com/en/docs/agents-and-tools/tool-use/token-efficient-tool-use) feature. It is supported by default on all Claude 4 models.
