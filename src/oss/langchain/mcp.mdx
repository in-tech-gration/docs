...
<Note>
    ...
    To enable session support, see the [stateful sessions](#stateful-sessions) section.
</Note>
...
Server runs as an independent process handling HTTP requests. Supports remote connections and multiple clients.
...
<Important>
    If you are serving your agent through a web server and intend to use it in a conversational setting, be careful with how you manage sessions. A naive setup will create a **new session for each request**, which prevents state from being preserved across turns.
</Important>
...
When an MCP tool returns `structuredContent`, the adapter wraps it in an [`MCPToolArtifact`](/docs/reference/langchain-mcp-adapters#MCPToolArtifact) and returns it as the tool's artifact. You can access this using the `artifact` field on the `ToolMessage`.
...
for message in result["messages"]:
    if message.type == "tool" and message.artifact:  # [!code highlight]
        # Access the MCPToolArtifact
        mcp_artifact = message.artifact  # [!code highlight]
        # Get the structured content
        structured_data = mcp_artifact.structured_content  # [!code highlight]
        print(f"Tool: {message.name}")
        print(f"Structured content: {structured_data}")
```

The `MCPToolArtifact` contains:
- `structured_content`: The parsed structured data returned by the MCP tool
...
- **Access LangGraph context**: Read user IDs, API keys, or other runtime values from `context`
- **Use the store**: Look up user preferences or persist data across conversations
- **Check agent state**: Make decisions based on the current conversation state
- **Control execution**: Implement retries, rate limiting, caching, or request patching

##### Accessing runtime context

When MCP tools are used within a LangGraph agent (via `create_agent`), interceptors receive access to the `ToolRuntime` context. This provides access to the tool call ID, state, config, and store—enabling powerful patterns for accessing user data, persisting information, and controlling agent behavior.
...
#### State updates and LangGraph commands

:::python
Interceptors can return `Command` objects to update agent state or control graph execution flow:
...
async def track_tool_usage(
    ...
    # Update state with tool usage counter
    return Command(  # [!code highlight]
        update={
            "tool_call_count": request.runtime.state.get("tool_call_count", 0) + 1,  # [!code highlight]
...
Use `Command` with `goto` to control graph flow—for example, to end the agent run early:

```python End agent run based on tool result
async def end_on_success(
    request: MCPToolCallRequest,
    handler,
):
    """End agent run if tool indicates task is complete."""
    result = await handler(request)

    if request.name == "submit_order" and "success" in str(result):
        return Command(
            update={
                "messages": [result],
...
:::python
Interceptors wrap tool execution to enable request/response modification, retry logic, and other cross-cutting concerns. They follow an "onion" pattern where the first interceptor in the list is the outermost layer.
