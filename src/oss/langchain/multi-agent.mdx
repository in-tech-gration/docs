...
Multi-agent systems enable you to build sophisticated applications by combining specialized agents. There are three main patterns:
...
| [**Handoffs**](#handoffs) | Agents transfer control to each other via state transitions or graph edges. **Decentralized** — the active agent changes dynamically. | Customer support, multi-domain conversations |
...
In **tool handoffs**, agents pass control through **state transitions**. Think of it as a state machine where the system tracks which agent is currently active, and that agent's configuration (system prompt, tools, etc.) determines the behavior. For example:
...
In **graph handoffs**, transitions between agents are defined by graph edges rather than tool calls. The flow is deterministic and defined at graph construction time.
...
<Tabs>
  ...
  <Tab title="Router">
    ...
    graph LR
        A[User] --> R[Router]
        R -->|condition A| B[Agent A]
        R -->|condition B| C[Agent B]
        R -->|condition C| D[Agent C]
        B --> E[Result]
        C --> E
        D --> E
    ```
...
Use graph handoffs when you have a **fixed sequence** of processing steps or **routing based on state**. The graph structure explicitly defines which agents can run and in what order, making execution predictable and easy to debug. Routing can be deterministic (based on state values) or use an LLM classifier.

Common examples:
- **Content pipelines**: research → draft → edit → publish
- **Data processing**: extract → validate → transform → load
- **Intent-based routing**: classify user intent and route to the appropriate specialist
...
<Tabs>
  ...
  <Tab title="Router">
    ...
    # Router function that classifies intent
    def route_by_intent(state: MessagesState) -> str:
        ...
        return intent  # Returns "billing", "technical", or "sales"
    ...
    workflow = (
        ...
        .add_conditional_edges(START, route_by_intent)
        .compile()
    )
    ```
    :::
    ...
    // Router function that classifies intent
    function routeByIntent(state: typeof MessagesAnnotation.State): string {
      // Use an LLM or classifier to determine intent
...
In the **skills** architecture, specialized capabilities are packaged as invokable "skills" that augment an agent's behavior. Skills use **progressive disclosure** — a context management technique where the agent loads information on-demand rather than upfront.
